...
async def google_login(
    ...
    user = await db.usuarios.find_one({"email": info["email"], "deleted_at": None})

    if user:
        # Actualizar foto y proveedor
...
async def reset_password(body: ResetPasswordRequest):
    ...
    await db.usuarios.update_one(
        {"_id": reset_doc["user_id"] if isinstance(reset_doc["user_id"], ObjectId)
